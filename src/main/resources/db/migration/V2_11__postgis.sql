...
UPDATE banner SET start_point = CASE WHEN start_latitude IS NOT NULL AND start_longitude IS NOT NULL THEN st_setsrid(st_makepoint(start_longitude, start_latitude), 4326) END;
...
UPDATE poi SET point = CASE WHEN latitude IS NOT NULL AND longitude IS NOT NULL THEN st_setsrid(st_makepoint(longitude, latitude), 4326) END;
...
UPDATE poi_audit SET point = CASE WHEN latitude IS NOT NULL AND longitude IS NOT NULL THEN st_setsrid(st_makepoint(longitude, latitude), 4326) END;
...
UPDATE place_coordinate SET point = st_setsrid(st_makepoint(longitude, latitude), 4326);
